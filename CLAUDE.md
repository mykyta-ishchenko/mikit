...
- **Project-agnostic.** Never reference a specific project's files, commands, or tools. Read the project's `CLAUDE.md` for its conventions; invoke a `learn-project` skill only if the project has one, otherwise fall back to `CLAUDE.md` and the files it references. Say "the project's verify command", not a literal command.
...
- Follow the `superpowers:writing-skills` skill when creating or editing a skill.
